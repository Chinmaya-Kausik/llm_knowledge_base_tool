...
def ingest_url(vault_root: Path, url: str, download_images: bool = True) -> dict:
    ...
    if download_images:
        media_dir = vault_root / "raw" / "media"
        original = content
...
def ingest_text(vault_root: Path, text: str, title: str) -> dict:
...
def classify_inbox_item(vault_root: Path, source: str, destination: str) -> dict:
    """Move a file from raw/inbox/ to the appropriate raw/ subdirectory.

    Args:
        source: Relative path from vault root (e.g., "raw/inbox/foo.md")
        destination: Relative path from vault root (e.g., "raw/articles/foo.md")

    Returns: {old_path, new_path}
    """
    src = vault_root / source
    dst = vault_root / destination
