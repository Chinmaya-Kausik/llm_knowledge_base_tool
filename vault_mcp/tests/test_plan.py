...
@pytest.fixture
def vault_app():
    """Create a test vault with plan files and return a test client."""
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        # Patch VAULT_ROOT and import app
        with patch("vault_mcp.web.VAULT_ROOT", root):
            from vault_mcp.web import app
            client = TestClient(app)
            yield client, root, plan_file


def test_api_get_plan(vault_app):
    """GET /api/plan returns the most recent plan file."""
    client, root, plan_file = vault_app
...
def test_api_put_plan(vault_app):
...
def test_api_put_plan_invalid_path(vault_app):
...
def test_api_delete_plan(vault_app):
...
def test_api_delete_plan_invalid_path(vault_app):
...
def test_checkbox_round_trip(vault_app):
