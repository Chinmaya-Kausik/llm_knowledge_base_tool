...
def vault_client(tmp_path):
    """Create a test vault and return (client, root)."""
    root = tmp_path / "vault"
    root.mkdir()
...
class TestSearchAPI:
    def test_search_content_mode(self, vault_client):
    ...
    def test_search_name_mode_file(self, vault_client):
        client, root = vault_client
    ...
    def test_search_name_mode_folder(self, vault_client):
        client, root = vault_client
    ...
    def test_search_both_mode(self, vault_client):
        client, root = vault_client
    ...
    def test_search_scope_wiki(self, vault_client):
        client, root = vault_client
    ...
    def test_search_file_glob(self, vault_client):
        client, root = vault_client
...
class TestBulkPages:
    def test_bulk_pages_returns_content(self, vault_client):
        client, root = vault_client
    ...
    def test_bulk_pages_missing_ids(self, vault_client):
        client, root = vault_client
    ...
    def test_bulk_pages_empty_list(self, vault_client):
        client, root = vault_client
...
class TestChatSaveAPI:
    def test_save_chat_via_api(self, vault_client):
        client, root = vault_client
        messages = [
    ...
    def test_save_chat_empty_messages(self, vault_client):
        client, root = vault_client
        r = client.post("/api/chat/save", json={
            "session_id": "empty",
            "messages": [],
    ...
    def test_save_chat_creates_chats_dir(self, vault_client):
        client, root = vault_client
...
class TestPlanSecurity:
    def test_put_plan_traversal_dotdot(self, vault_client):
        client, root = vault_client
    ...
    def test_delete_plan_traversal(self, vault_client):
        client, root = vault_client
    ...
    def test_put_plan_empty_path(self, vault_client):
        client, root = vault_client
        r = client.put("/api/plan", json={"path": "", "content": "test"})
        assert r.json()["ok"] is False

    def test_put_plan_outside_plans_dir(self, vault_client):
        client, root = vault_client
        r = client.put("/api/plan", json={
...
class TestOpenExternal:
    def test_open_external_missing_path(self, vault_client):
        client, root = vault_client
        r = client.post("/api/open-external", json={"path": ""})
        assert r.json()["ok"] is False

    def test_open_external_nonexistent_file(self, vault_client):
        client, root = vault_client
        r = client.post("/api/open-external", json={"path": "nonexistent.md"})
        assert r.json()["ok"] is False

    def test_open_external_traversal(self, vault_client):
        client, root = vault_client
        r = client.post("/api/open-external", json={"path": "../../etc/passwd"})
        assert r.json()["ok"] is False

    def test_open_external_valid_file(self, vault_client):
        client, root = vault_client
...
class TestTerminalEndpoint:
    def test_terminal_endpoint_registered(self, vault_client):
        client, root = vault_client
        import vault_mcp.web as web_module
...
class TestMediaServing:
    def test_media_serves_existing_file(self, vault_client):
        client, root = vault_client
        r = client.get("/media/wiki/concepts/alpha/code.py")
        assert r.status_code == 200
        assert "def alpha" in r.text

    def test_media_404_missing_file(self, vault_client):
        client, root = vault_client
...
class TestSettingsAPI:
    def test_get_settings(self, vault_client):
        client, root = vault_client
    ...
    def test_put_settings_vault_root(self, vault_client):
        client, root = vault_client
        new_root = str(root / "wiki")
        r = client.put("/api/settings", json={"vault_root": new_root})
        assert r.status_code == 200
...
class TestTreeStructure:
    def test_tree_includes_mtime(self, vault_client):
        client, root = vault_client
    ...
    def test_tree_has_correct_hierarchy(self, vault_client):
        client, root = vault_client
        tree = client.get("/api/tree").json()
...
class TestGraphStructure:
    def test_graph_has_edges_structure(self, vault_client):
        client, root = vault_client
    ...
    def test_graph_nodes_have_required_fields(self, vault_client):
        client, root = vault_client
        data = client.get("/api/graph").json()
    ...
    def test_graph_top_nodes_subset(self, vault_client):
        client, root = vault_client
        data = client.get("/api/graph").json()
