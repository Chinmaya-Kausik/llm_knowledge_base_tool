"""Functional tests for the vault chat system.
...
To run:  uv run pytest vault_mcp/tests/test_functional.py -v
"""
...
def _make_vault(tmp_path: Path) -> Path:
    """Bootstrap a minimal vault with searchable content."""
...
def vault_client(tmp_path):
    """Yield (TestClient, vault_root) with VAULT_ROOT pointed at temp vault."""
...
class TestChatWithClaude:
    ...
    def test_basic_chat_flow(self, vault_client):
        """Connect, init, send message, receive text events, receive done."""
        client, root = vault_client
    ...
    def test_tool_use_events(self, vault_client):
        """Ask Claude to grep the vault; verify tool_use and tool_result arrive."""
        client, root = vault_client
        ...
        with client.websocket_connect("/ws/chat") as ws:
            ...
            ws.send_json({
                "type": "message",
                "text": (
                    "Use the ripgrep_search tool to search for 'attention' in the vault. "
    ...
    def test_multi_turn_context(self, vault_client):
    ...
    def test_stop_and_resume(self, vault_client):
...
class TestChatSave:
    def test_save_basic_messages(self, vault_client):
    ...
    def test_save_empty_messages_fails(self, vault_client):
...
class TestChatSaveCompleteness:
    def test_save_includes_tool_calls_and_text_after(self, vault_client):
        ...
        messages = [
            ...
            {"role": "assistant", "content": "I found references to transformers in the vault."},
    ...
    def test_save_with_thinking_and_subagent(self, vault_client):
...
class TestSettings:
    def test_get_settings_returns_expected_fields(self, vault_client):
        """GET /api/settings returns vault_root and claude_authenticated."""
        client, root = vault_client
        ...
        assert isinstance(data["vault_root"], str)
        assert isinstance(data["claude_authenticated"], bool)
        # vault_root should point to our temp vault
        assert str(root) in data["vault_root"]
...
class TestSearch:
    def test_search_returns_results(self, vault_client):
    ...
    def test_search_scoped_to_wiki(self, vault_client):
    ...
    def test_search_no_results(self, vault_client):
...
class TestGraph:
    def test_graph_returns_structure(self, vault_client):
    ...
    def test_graph_contains_vault_pages(self, vault_client):
        """Graph nodes should include pages from the vault."""
        client, root = vault_client
...
class TestPageRead:
    def test_page_returns_frontmatter_and_content(self, vault_client):
    ...
    def test_page_content_has_no_yaml_markers(self, vault_client):
    ...
    def test_page_not_found(self, vault_client):
    ...
    def test_page_read_code_file(self, vault_client):
...
class TestWebSocketProtocol:
    ...
    def test_connect_and_init(self, vault_client):
    ...
    def test_message_before_init_errors(self, vault_client):
    ...
    def test_empty_message_errors(self, vault_client):
    ...
    def test_invalid_json_errors(self, vault_client):
    ...
    def test_unknown_type_does_not_crash(self, vault_client):
    ...
    def test_init_generates_session_id_if_missing(self, vault_client):
