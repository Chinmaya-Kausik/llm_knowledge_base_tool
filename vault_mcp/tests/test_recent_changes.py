"""Tests for recent changes: _render_messages, chat append, vault root validation,
...
from vault_mcp.tools.compile import _render_messages, save_chat_transcript
...
class TestChatAppendAPI:
    def test_append_to_existing_chat(self, vault_client):
    ...
    def test_append_invalid_path(self, vault_client):
        client, root = vault_client
        r = client.post("/api/chat/append", json={
            "path": "wiki/README.md",
    ...
    def test_append_nonexistent_file(self, vault_client):
        client, root = vault_client
        r = client.post("/api/chat/append", json={
    ...
    def test_append_empty_messages(self, vault_client):
        client, root = vault_client
        r = client.post("/api/chat/append", json={
...
class TestUpdateTitleAPI:
    def test_update_title(self, vault_client):
        client, root = vault_client
    ...
    def test_update_title_invalid_path(self, vault_client):
        client, root = vault_client
        r = client.post("/api/chat/update-title", json={
            "path": "wiki/README.md",
            "title": "hack",
...
class TestVaultRootValidation:
    def test_relative_path_rejected(self, vault_client):
        client, root = vault_client
        r = client.put("/api/settings", json={"vault_root": "Users/someone/vault"})
        data = r.json()
        assert data["ok"] is False
        assert "absolute" in data["error"].lower() or "absolute" in data.get("error", "")

    def test_nonexistent_path_rejected(self, vault_client):
        client, root = vault_client
        r = client.put("/api/settings", json={"vault_root": "/tmp/definitely-does-not-exist-12345"})
        data = r.json()
        assert data["ok"] is False

    def test_valid_path_accepted(self, vault_client):
        client, root = vault_client
        r = client.put("/api/settings", json={"vault_root": str(root)})
        data = r.json()
        assert data["ok"] is True

    def test_tilde_path_expanded(self, vault_client):
        client, root = vault_client
        # ~ expands to home dir which should exist
        r = client.put("/api/settings", json={"vault_root": "~"})
        data = r.json()
        # Should either succeed (home dir exists) or fail (not a vault), but not crash
        assert "ok" in data


class TestRestartEndpoint:
    def test_restart_resolves_vault_root(self, vault_client):
        client, root = vault_client
