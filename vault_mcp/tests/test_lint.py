...
from vault_mcp.tools.lint import (
    check_terminology,
...
def _add_wiki_page(vault: Path, rel_path: str, title: str, content: str,
...
def test_validate_links_no_broken():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _add_wiki_page(vault, "wiki/concepts/a.md", "A", "Links to [[B]].")
        _add_wiki_page(vault, "wiki/concepts/b.md", "B", "No links.")
        broken = validate_links(vault)
        assert len(broken) == 0
...
def test_validate_links_broken():
    with tempfile.TemporaryDirectory() as tmp:
        vault = _make_vault(tmp)
        _add_wiki_page(vault, "wiki/concepts/a.md", "A", "Links to [[NonExistent]].")
        broken = validate_links(vault)
        assert len(broken) == 1
...
def test_validate_links_alias_resolution():
    with tempfile.TemporaryDirectory() as tmp:
        vault = _make_vault(tmp)
        _add_wiki_page(vault, "wiki/concepts/transformer.md", "Transformers",
                       "Content.", aliases=["Vaswani architecture"])
        _add_wiki_page(vault, "wiki/concepts/bert.md", "BERT",
...
def test_find_stale_pages_hash_mismatch():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _add_wiki_page(vault, "wiki/concepts/derived.md", "Derived", "Content.",
...
def test_find_orphan_pages():
    with tempfile.TemporaryDirectory() as tmp:
        vault = _make_vault(tmp)
        _add_wiki_page(vault, "wiki/concepts/a.md", "A", "Links to [[B]].")
        _add_wiki_page(vault, "wiki/concepts/b.md", "B", "No links out.")
        _add_wiki_page(vault, "wiki/concepts/c.md", "C", "Orphan, no links in.")

        orphans = find_orphan_pages(vault)
...
def test_find_missing_concepts():
    with tempfile.TemporaryDirectory() as tmp:
        vault = _make_vault(tmp)
        _add_wiki_page(vault, "wiki/concepts/a.md", "A", "Content.",
                       related=["[[B]]", "[[Missing Concept]]"])
        _add_wiki_page(vault, "wiki/concepts/b.md", "B", "Content.")

        missing = find_missing_concepts(vault)
...
def test_check_terminology_known_terms():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        unknown = check_terminology(vault, "wiki/concepts/a.md")
...
def test_generate_health_report():
    with tempfile.TemporaryDirectory() as tmp:
        vault = _make_vault(tmp)
        _add_wiki_page(vault, "wiki/concepts/a.md", "A", "Links to [[Missing]].")

        result = generate_health_report(vault)
        assert result["broken_links"] == 1
        assert result["total_issues"] >= 1
        assert (vault / "wiki" / "meta" / "health.md").exists()
