...
function bringToFront(el) {
  ...
  // When split/fullpage overlays are open, move panel to document.body
  // so it's not trapped behind the overlay's opaque background
  if ((splitOverlay || expandedCard) && el.parentNode !== document.body) {
...
async function expandCardFullPage(card, highlightQuery) {
  ...
  if (path.endsWith('.tex')) {
    ...
    mainBtn.onclick = async () => {
      ...
      // Collect all .tex files from the graph data (already loaded)
      const texFiles = [];
      if (graphData?.nodes) {
        for (const n of graphData.nodes) {
          if (n.data.path?.endsWith('.tex')) texFiles.push(n.data.path);
        }
      }
      // Fallback: if graph doesn't have them, at least show the current file
      if (texFiles.length === 0) texFiles.push(path);
      texFiles.sort();

      // Position dropdown below the button, on document.body
      const rect = mainBtn.getBoundingClientRect();
      const picker = document.createElement('div');
      picker.id = 'tex-main-picker';
      picker.style.cssText = `position:fixed;top:${rect.bottom + 4}px;left:${rect.left}px;background:var(--bg);border:1px solid var(--border);border-radius:6px;box-shadow:var(--shadow-md);z-index:500;max-height:300px;overflow-y:auto;min-width:280px;-webkit-backdrop-filter:none;backdrop-filter:none`;
      const currentMain = getMainTex();
      for (const f of texFiles) {
        const item = document.createElement('div');
        item.style.cssText = 'padding:8px 14px;cursor:pointer;font-size:13px;display:flex;align-items:center;gap:8px;white-space:nowrap';
        const name = f.split('/').pop();
        const dir = f.split('/').slice(0, -1).join('/');
        item.innerHTML = `<span style="color:var(--accent)">${f === currentMain ? '★' : '&nbsp;&nbsp;'}</span><span>${name}</span><span style="color:var(--text-dim);font-size:11px;margin-left:auto;overflow:hidden;text-overflow:ellipsis">${dir}</span>`;
      ...
      setTimeout(() => {
        const close = (e) => { if (!picker.contains(e.target) && e.target !== mainBtn) { picker.remove(); document.removeEventListener('mousedown', close); } };
