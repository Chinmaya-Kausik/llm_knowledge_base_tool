...
from loom_mcp.lib.links import find_backlinks, parse_links
from loom_mcp.lib.registry import load_registry


def validate_links(loom_root: Path) -> list[dict]:
    ...
    for md_file in wiki_dir.rglob("*.md"):
        ...
        rel_path = str(md_file.relative_to(loom_root))
...
def find_stale_pages(loom_root: Path) -> list[dict]:
    ...
    for md_file in wiki_dir.rglob("*.md"):
        ...
        for source_ref in sources:
            source_path = loom_root / source_ref.get("path", "")
            expected_hash = source_ref.get("hash", "")

            if not source_path.exists():
                stale.append({
                    "page": str(md_file.relative_to(loom_root)),
...
def find_orphan_pages(loom_root: Path) -> list[str]:
...
def find_missing_concepts(loom_root: Path) -> list[str]:
...
def check_terminology(loom_root: Path, page_path: str) -> list[str]:
...
def generate_health_report(loom_root: Path) -> dict:
    """Run all checks and write wiki/meta/health.md.

    Returns: {broken_links, stale_pages, orphans, missing_concepts, total_issues}
    """
    broken_links = validate_links(loom_root)
    stale_pages = find_stale_pages(loom_root)
    orphans = find_orphan_pages(loom_root)
    missing_concepts = find_missing_concepts(loom_root)
    ...
    repo_size_result = subprocess.run(
        ["du", "-sh", str(loom_root)], capture_output=True, text=True
    )
    if repo_size_result.returncode == 0:
        size_str = repo_size_result.stdout.split()[0]
        if size_str.endswith("G") and float(size_str[:-1]) > 1:
            scaling_alerts.append(f"SCALING: Loom is {size_str}. Consider git-lfs for large binaries.")

    # Stale READMEs count
    from loom_mcp.tools.compile import get_stale_readmes
    stale_readmes = get_stale_readmes(loom_root)
    ...
    content = "\n".join(lines)
    health_path = loom_root / "wiki" / "meta" / "health.md"
    health_metadata = {
